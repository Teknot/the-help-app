...
import { ADD_SERVICE } from "../utils/config";
...
export default function AddService({ navigation }) {
  ...
  const validate = () => {
    if (name === "" || name.length === 0) {
      ...
      addService(userObject);
    }
  };

  const addService = (object) => {
    setLoading(true);
    console.log(object)
    axios
      .post(ADD_SERVICE, object)
  ...
  return (
    <ScrollView style={styles.container} showsVerticalScrollIndicator={false}>
      <Header heading={"Add New Service"}></Header>
      <View style={styles.center_section}>
        ...
        <Button title={"Add Service"} onPress={validate} loading={loading}/>
      </View>
      <Toast />
    </ScrollView>
  );
